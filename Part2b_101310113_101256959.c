...
void mark_exams(int TA_ID) {
    ...
    while (1) {
        ...
        sem_wait(&shared->question_mutex); // Lock
        ...
        sem_post(&shared->question_mutex); // Unlock
...
void edit_rubric(int TA_ID, const char *rubric) {
    ...
    sem_wait(&shared->rubric_mutex); // Lock rubric
    ...
    sem_post(&shared->rubric_mutex); // Unlock rubric for next TA
}
...
int main(int argc, char *argv[]) {
    ...
    // initialize semaphores for Part 2b
    if (sem_init(&shared->question_mutex, 1, 1) == -1) {
        perror("sem_init question_mutex");
        shmdt(shared);
        shmctl(shmid, IPC_RMID, NULL);
        return 1;
    }
    if (sem_init(&shared->rubric_mutex, 1, 1) == -1) {
        perror("sem_init rubric_mutex");
        sem_destroy(&shared->question_mutex);
        shmdt(shared);
        shmctl(shmid, IPC_RMID, NULL);
        return 1;
    }
    ...
    for (int exam_index = 1; exam_index <= NUM_EXAMS; exam_index++) {
        ...
        // Fork a loader TA process to load this exam file
        ...
        // Parent: wait for loader TA to finish loading the exam
    ...
    // destroy semaphores for Part 2b
    sem_destroy(&shared->question_mutex);
    sem_destroy(&shared->rubric_mutex);
